...
ch = str(b64decode("QEZsdWtvc2Fh"))[2:15]
gc = str(b64decode("QFVwdXR0c3VwcG9ydA=="))[2:13]
...
async def update_restart_msg(chat_id, msg_id):
    ...
    message = (
        f"**Ewe-Userbot v`{var.BOT_VER}` is back up and running!**\n\n"
        f"**Telethon:** `{vsc}`\n"
        f"**Python:** `{python_version()}`\n"
    )
    await Rewe.edit_message(chat_id, msg_id, message)
    return True
