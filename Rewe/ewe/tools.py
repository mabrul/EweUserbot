...
from Rewe import Rewe, LOGS
from Rewe.ewe.format import md_to_text, paste_message
...
async def extract_time(putt, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]
        if not time_num.isdigit():
            await putt.edit("Jumlah waktu yang ditentukan tidak valid.")
            return None
        if unit == "s":
...
async def is_admin(chat_id, user_id):
    req_jo = await Uputt(GetParticipantRequest(channel=chat_id, user_id=user_id))
