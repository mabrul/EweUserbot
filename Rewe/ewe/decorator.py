...
from .tools import ReweChanger
from .toolsputt import eod, eor
    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            ewe_reg = sudo_reg = re.compile(pattern)
        else:
            iamuput_ = "\\" + var.CMD_HANDLER
            sudo_ = "\\" + var.SUDO_HANDLER
            ewe_reg = re.compile(ewe_ + pattern)
...
def chataction(**args):
    def decorator(func):
        if Rewe:
            Rewe.add_event_handler(func, events.ChatAction(**args))
        return func
