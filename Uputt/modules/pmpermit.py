...
from Uputt import CMD_HELP, COUNT_PM, LASTMSG, LOGS, Uputt
from Uputt.database.permit import (
    approve,
    del_permit_message,
    disapprove,
    get_permit_message,
...
from Uputt.database.variable import del_var, get_var, set_var
from Uputt.events import uputt_cmd
from Uputt.iamuput import UputtChanger, eod, eor
...
│ㅤㅤ𖣘 𝚄𝙿𝚄𝚃𝚃-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 𖣘ㅤㅤ   
╚═════════════════════╝
"""
)


@Uputt.on(events.NewMessage(incoming=True))
async def permitpm(event):
    ...
    if (
        ...
        apprv = UputtChanger(is_approved())
            ...
            if event.chat_id in DEVS:
                if event.chat_id not in apprv:
                    try:
                        ...
                        await Uputt.send_message(var.BOTLOG_CHATID, f"**#AUTO_APPROVED_DEVELOPER**\n\n👑 **Developer:** [{sender.first_name}](tg://user?id={sender.id})\n💬 `Developer Daren-Ucelbot Telah Mengirimi Anda Pesan...`")
                        await Uputt.send_message(
                            event.chat_id,
                            f"**Menerima Pesan!!!**\n**Terdeteksi [{sender.first_name}](tg://user?id={sender.id}) Adalah Developer Uputt-Userbot**"
                        )
                        return
...
@Uputt.on(events.NewMessage(outgoing=True))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"notifoff$"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"notifon$"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"block$"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"unblock$"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"permit (on|off)(?: |$)(\w*)"))
...
@Uputt.on(uputt_cmd(outgoing=True, pattern=r"(set|get|reset) pmpermit(?: |$)(\w*)"))
