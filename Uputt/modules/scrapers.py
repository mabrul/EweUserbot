...
from Uputt.iamuput.FastTelethon import upload_file
...
@uputt_cmd(pattern="currency ([\\d\\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@uputt_cmd(pattern="google ([\\s\\S]*)")
async def gsearch(q_event):
    putt = await eor(q_event, "**Memproses...**")
...
@uputt_cmd(pattern=r"lang (tr|tts) (.*)")
...
@uputt_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
...
async def ocr(event):
    ...
    downloaded_file_name = await Uputt.download_media(
...
@uputt_cmd(pattern=r"makeqr(?: |$)([\s\S]*)")
...
CMD_HELP.update(
    {
        "Urban": f"**Plugin : **`Urban Dictionary`\
