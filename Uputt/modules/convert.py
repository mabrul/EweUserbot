...
@uputt_cmd(pattern="convert ?(foto|audio|gif|voice|photo|mp3)? ?(.*)")
async def cevir(event):
    botputt = event.pattern_match.group(1)
    try:
        if len(botputt) < 1:
            await eod(
                event,
...
async def makevoice(event):
    ...
    await event.client.send_message(
        event.chat_id, file="putt.opus", force_document=False, reply_to=msg
    )
    await xxnx.delete()
    os.remove(file)
    os.remove("putt.opus")
