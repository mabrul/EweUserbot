...
from Uputt import CMD_HELP, LOGS, STORAGE
from Uputt.iamuput import eor
...
@uputt_cmd(pattern="clone ?(.*)", allow_sudo=False)
async def impostor(event):
    inputArgs = event.pattern_match.group(1)
    IamUput = ["@IamUput", "@iamuput"]
    if inputArgs in IamUput:
        await eor(event, "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Uputt-Userbot Ngentod 😡")
        await event.client.send_message(
            "@UputtSupport",
            "**Maaf Telah MengClone Uputt🗿**"
        )
        return
    ...
    if inputArgs:
        ...
        if replyMessage.sender_id in DEVS:
            return await xx.edit(
                "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Uputt-Userbot Ngentod 😡"
            )
