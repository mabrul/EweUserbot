...
from Uputt.iamuput import uputt_cmd, bash, deEmojify
...
async def ngethreat(event):
    ...
    if replied.media:
        ...
        await event.edit("reply to a supported media file")
        return
    download_location = await Uputt.download_media(replied, var.TEMP_DOWNLOAD_DIRECTORY)
    ...
    await event.delete()
    await Uputt.send_file(event.chat_id, file, reply_to=replied)


@uputt_cmd(pattern="trash(?: |$)(.*)")
...
async def ngetrap(e):
    ...
    await e.delete()
    await Uputt.send_file(e.chat_id, file, reply_to=replied)


# Ported by @AshSTR


@uputt_cmd(pattern="fgs ((.*) ; (.*))")
async def FakeGoogleSearch(event):
    ...
    font1 = ImageFont.truetype(
        "Uputt/iamuput/styles/ProductSans-BoldItalic.ttf", 20)
    font2 = ImageFont.truetype(
        "Uputt/iamuput/styles/ProductSans-Light.ttf", 23)
