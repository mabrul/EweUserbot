...
from Uputt.database.muted import (
    add_mute,
    add_gmute,
    cek_mute,
    cek_gmute,
...
@uputt_cmd(pattern="setgpic( -s| -d)$", group_only=True)
...
@uputt_cmd(pattern="promote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@uputt_cmd(pattern="demote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@uputt_cmd(pattern="ban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@uputt_cmd(pattern="unban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@uputt_cmd(pattern="mute(?: |$)(.*)", group_only=True)
...
async def spider(spdr):
    ...
    muting = UputtChanger(cek_mute(self_user.id))
...
@uputt_cmd(pattern="unmute(?: |$)(.*)", group_only=True)
...
@uputt_cmd(pattern="ungmute(?: |$)(.*)", group_only=True)
...
async def ungmoot(un_gmute):
    ...
    if not user:
        return
    await uputt.edit("**Membuka Global Mute Pengguna...**")
    ungmuting = UputtChanger(cek_gmute(me.id))
    if user.id in ungmuting:
        await uputt.edit("**KESALAHAN!** Pengguna Sudah Tidak Di Bisukan Global...")
        del_gmute(me.id, user.id)
        return
    else:
        await uputt.edit("**BERHASIL! Pengguna Sudah Tidak Dibisukan**", time=10)


@uputt_cmd(pattern="gmute(?: |$)(.*)", group_only=True)
...
async def gspider(gspdr):
    try:
        ...
        uputt = await eor(gspdr, '**Memproses...**')
...
'''
        ...
        await eor(uputt, f'{format_exc()}')


@uputt_cmd(pattern="zombies(?: |$)(.*)", group_only=True)
...
@uputt_cmd(pattern="admins$", group_only=True)
...
@uputt_cmd(pattern="kick(?: |$)(.*)", group_only=True)
