...
from Uputt import Uputt, LOGS, StartTime, bot
from Uputt.database.bot_blacklist import check_is_black_list
from Uputt.database.bot_start import (
    add_starter_to_db,
    get_all_starters,
    get_starter_details,
)
from Uputt.database.variable import get_var
from Uputt.iamuput import _format, asst_cmd, callback, reply_id, set_var_value
...
async def check_bot_started_users(user, event):
    if user.id == (await Uputt.get_me()).id:
        return
...
async def pmclose(event):
    if event.query.user_id == (await Uputt.get_me()).id:
        await event.delete()
...
async def pmbot(event):
    await event.delete()
    UputtUBOT = await bot.get_me()
    botusername = UputtUBOT.username
    if event.query.user_id == (await Uputt.get_me()).id:
...
async def strone(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"**Silahkan Kirimkan {var} Telethon anda**\n\nGunakan /cancel untuk membatalkan."
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
...
async def strtri(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"**Silahkan Kirimkan {var} Telethon anda dari**\n\nGunakan /cancel untuk membatalkan."
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
