...
from Uputt.iamuput import eod, eor
from Uputt.events import register
from Uputt.modules.putping import absen
from Uputt.iamuput import uputt_cmd, get_user_from_event

from . import cmd


@uputt_cmd(pattern="userid$")
...
@uputt_cmd(pattern="inviteall ?(.*)")
async def get_users(event):
    uputt = event.text[11:]
    chat_uputt = uputt_.lower()
    restricted = ["@UputtSupport", "@uputtsupport"]
    if chat_uputt in restricted:
        await eor(event, "**Anda tidak dapat Mengundang Anggota dari sana.**")
        await event.client.send_message(
            -1001608701614, "**Maaf Telah Mencuri Member dari Sini.**"
        )
        return
    if not uputt_:
        return await eor(
    ...
    if event.is_private:
        return await uputt.edit("**Tidak bisa Menambahkan Member di sini Harap ketik di Grup Chat**")
    s = 0
    f = 0
    error = "None"
    await uputt.edit("**Terminal Status**\n\n`Sedang Mengumpulkan Pengguna...`")
...
CMD_HELP.update(
    {
        ...
        \n  •  **Contoh : **{cmd}link @iamuput Ganteng\
    "
    }
)
